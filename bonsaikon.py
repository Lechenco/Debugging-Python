...
class Invariants:
    def __init__(self):
        ...
        #defaultdict helper function
        import collections
        def multidict(): 
            return collections.defaultdict(multidict)
        self.vars = multidict()
    ...
    def track(self, frame, event, arg):
        if event == "call" or event == "return":
            ...
            if frame.f_code.co_name == "square_root":
                self.vars["square_root"]["call"]["x"].track(frame.f_locals["x"])
                self.vars["square_root"]["call"]["eps"].track(frame.f_locals["eps"])
                if event == "return":
                    self.vars["square_root"]["return"]["y"].track(frame.f_locals["y"])
                    self.vars["square_root"]["return"]["ret"].track(arg)
